    ...
    async createNote() {
        ...
        try {
            ...
            if (response.data != "You have reached your note limit") {
                ...
                let finalHeight // browser needs a specifc height to transition to, you can't transition to 'auto' height
                let newlyCreated = document.querySelector("#my-notes li")

                //give the browser 30 miliseconds to have invisible element added to the DOM before moving on
                setTimeout(function () {
                    finalHeight = `${newlyCreated.offsetHeight}px`
                    newlyCreated.style.height = "0px"
                }, 30)

                //give the browser another 20 miliseconds to count the height of invisible element before moving on
                setTimeout(function () {
                    newlyCreated.classList.remove("fade-in-calc")
                    newlyCreated.style.height = finalHeight
                }, 50)

                //wait the duration of the CSS transition before removing the hardcoded calculeted height from element so that our design is responsive once again
                setTimeout(function () {
